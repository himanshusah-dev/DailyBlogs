...
function Home() {
  ...
  async function fetchData() {
    try {
      const response = await axios.get(`${apiUrl}/api/v1/blog/allblogs`);

      // console.log("Data:", response.data.content[0]);

      setAllBlog(response.data);
      setFilterBlog(response.data);
    } catch (error) {
  ...
  return (
    <div>
      ...
      <div className="">
        <div>
          <h1 className="text-3xl text-center font-bold m-1 ">All Blogs</h1>
          ...
          <div className=" flex ">
            ...
            <button
              ...
              onClick={() => {
                const filter = allblog.filter((res) =>
              ...
              filterBlog.map((blog, index) => (
                <div key={index} className="  ">
                  ...
                  <div className="ml-20  mr-20 flex flex-col justify-around bg-gray-200 p-3 min-h-32 rounded-md ">
                    ...
                    <button
                      className="bg-green-500 px-4 py-2 text-white rounded content-center justify-center items-center w-32 hover:bg-green-600 mt-10"
                      onClick={() =>
